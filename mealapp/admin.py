...
class MemberInfoAdmin(admin.ModelAdmin):
    list_display    = ['member_name','address','mobile','email','status',]
    search_fields   = ['member_name','address','mobile','email','status',]
    list_filter     = ['status',]

class AddBazarAdmin(admin.ModelAdmin):
    list_display    = ['author','total_bazar','bazar_date','status']
    search_fields   = ['author','total_bazar','bazar_date','status']
    list_filter     = ['status',]

class AddAmountAdmin(admin.ModelAdmin):
    list_display    = ['member_name','amount','add_date','status']
    search_fields   = ['member_name','amount','add_date','status']
    list_filter     = ['status',]

class DailyMealAdmin(admin.ModelAdmin):
    list_display    = ['member_name', 'no_of_meal', 'add_date', 'status']
    search_fields   = ['member_name', 'no_of_meal', 'add_date', 'status']
    list_filter     = ['status',]

admin.site.register(models.MemberInfo,MemberInfoAdmin)
admin.site.register(models.AddBazar,AddBazarAdmin)
admin.site.register(models.AddAmount,AddAmountAdmin)
admin.site.register(models.DailyMeal,DailyMealAdmin)
